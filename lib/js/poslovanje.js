...
function zahtjevi(id,dodatno){
	var url=baseUrl+'controler/zahtjevi.php';
		if(id==1){	
			var data=$("form#noviZahtjevWifi").serialize();
			$.ajax({
	                type: "post",
					data: data+"&dodatno="+dodatno+"&action="+id,
					url: url,
					success: function(text) {
						
							if(text=="Zahtjev za besplatni internet predan."){
								alert(text);
								loadScreen('profile',0);	
							}						
						   }
			...
			$.ajax({
				...
				success: function(text) {
					...
					alert(text);
					if(text=="Zahtjev uspješno deaktiviran"){
						loadScreen('profile',0);	
					}			
